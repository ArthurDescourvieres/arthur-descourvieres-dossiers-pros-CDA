...
export function LuminaProvider({ children }: { children: ReactNode }) {
  ...
  const openCtxMenu = useCallback((items: CtxMenuEntry[], x: number, y: number) => {
    setCtxMenu({ open: true, x, y, items })
  }, [])
  const closeCtxMenu = useCallback(() => setCtxMenu((c) => ({ ...c, open: false })), [])

  const [modal, setModal] = useState<ModalState>({ open: false, body: null })
  const openModal = useCallback((body: ReactNode) => setModal({ open: true, body }), [])
