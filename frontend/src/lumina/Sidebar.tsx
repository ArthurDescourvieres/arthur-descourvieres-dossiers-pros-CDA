...
function NavItem({ node, depth }: { node: NavNode; depth: number; isLast: boolean }) {
    ...
    (clientX: number, clientY: number) => {
      ...
      items.push({
        ...
        action: () => {
          ...
          lumina.bumpNav()
          lumina.showToast(node.starred ? 'Added to favourites' : 'Removed from favourites', 'star')
        },
      })
      if (!isVirtual)
        items.push({
      ...
      items.push({
        label: 'Move to…',
        icon: 'move',
        action: () => lumina.showToast('Pick a destination from the sidebar', 'move'),
      })
      items.push('-')
      if (!isVirtual)
        items.push({
          ...
          action: () => {
            ...
            if (node.mapId && lumina.currentPageId === node.mapId) lumina.openVirtual('trash')
            lumina.bumpNav()
  ...
  const onClick = (e: RMouseEvent<HTMLDivElement>) => {
    ...
    if (target.closest('[data-role="label"]')?.getAttribute('contenteditable') === 'true') return
    if (isFolder) {
        ...
        >
          <div>
            {node.expanded
              ...
              : null}
            {node._pendingNew ? <NavNewInput parent={node} depth={depth + 1} /> : null}
          </div>
        </div>
      ) : null}
      {!hasChildren && node._pendingNew ? <NavNewInput parent={node} depth={depth + 1} /> : null}
  ...
  return (
    ...
    >
      <span className="w-[16px] h-[16px] grid place-items-center text-[var(--color-teal)] [&>svg]:w-[14px] [&>svg]:h-[14px]">
        <LuminaIcon name={parent._pendingNew?.type === 'folder' ? 'folder' : 'file-text'} />
      </span>
      <input
        ref={inputRef}
        type="text"
        placeholder={parent._pendingNew?.type === 'folder' ? 'Folder name' : 'Untitled note'}
        value={value}
          ...
          : (() => {
              ...
              const mutatePages = (
                lumina as unknown as {
                  mutatePages: (fn: (p: Record<string, unknown>) => Record<string, unknown>) => void
                }
              ).mutatePages
              mutatePages((p) => ({ ...p, [id]: page }))
      ...
      <div className="relative z-[1] flex flex-col h-full py-[var(--space-4)] px-[var(--space-3)]">
        ...
        <nav className="nav flex flex-col gap-px flex-1 overflow-y-auto overflow-x-hidden pb-[var(--space-6)] relative">
          {lumina.navTree.map((n, i) => (
            <NavItem key={n.uid} node={n} depth={0} isLast={i === lumina.navTree.length - 1} />
          ))}
        </nav>
        ...
        <div className="relative z-[1] mt-auto pt-[var(--space-3)] px-[var(--space-2)] border-t border-[var(--color-line)]">
          <button className="flex items-center gap-[var(--space-3)] py-[var(--space-2)] px-[var(--space-3)] rounded-[var(--r-md)] w-full text-left transition-colors duration-[180ms] hover:bg-[var(--color-overlay-weak)]">
            ...
            <div>
              <div className="text-[13px] text-[var(--color-text)]">Elise Marchetti</div>
              <div className="text-[11px] text-[var(--color-text-faint)]">elise@lumina.work</div>
            </div>
        ...
        {pages.map((p) => (
          ...
          >
            ...
            <div style={{ textAlign: 'left' }}>
              <div style={{ fontSize: 13.5, color: 'var(--color-text)' }}>{p.title}</div>
